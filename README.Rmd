...
First we apply the `ii_summarize` function. This will take the columns specified in `value` and and apply the `fun` stratified according to `id`. In this case we want to calculate the mean meth value within each gene.
...
Next we use the `bootter` function. This will take the summarized data.frame calculated earlier along with the original dataset. The function with return the original data.frame with the new column attached to the end, which makes it ideal for piping to apply different methods to the same data.
...
adjusted <- bootter(data = sample_ii, 
            ...
            bootter(data_full =  sample_data,
                    value =  meth,
                    id = gene, reps = 100, method = "perm_v3")
adjusted
```
